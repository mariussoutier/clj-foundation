...
(deftask midje
  ...
  (bake (:use [bake.core :only [with-context]])
	...
	[namespaces (remove #(.contains (str %1) "perf-") (concat (find-namespaces-in-dir (java.io.File. "test"))
...
(deftask perf
  ...
  (bake (:use [bake.core :only [with-context]])
	  (:require [clojure test string])
	[namespaces (concat (filter #(.contains (str %1) "perf-") (find-namespaces-in-dir (java.io.File. "test")))
		             (find-namespaces-in-dir (java.io.File. "src")))]
	  (with-context :test
	    )
  ))